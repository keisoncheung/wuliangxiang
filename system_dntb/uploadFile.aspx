...
<body topmargin="0">
    <form id="uploadFace" runat="server">
        ...
        <input language="javascript" onclick="javascript:newFile()" type="button" value='<%=ResourceManager.GetString("insertfile")%>' id="insertFile" /><br />
                      ...
                      <div style=" display:none">[ <%=ResourceManager.GetString("uploaduse")%>：<asp:label ID="useSpace" ForeColor="Red" runat=server />，<%=ResourceManager.GetString("have")%>：<asp:label ID="space" ForeColor="Red" runat=server /><%=ResourceManager.GetString("singlesize")%>：
...
<HeaderStyle ForeColor="WindowText" BackColor="Control" BorderWidth="1px" HorizontalAlign="Center" />
    ...
    <input language="javascript" onclick="window.close();" type="button" value="<%=ResourceManager.GetString("close")%>" /><br /><span style="display:none;"><a  href="#" onclick="if(is_ie){showModalDialog('Advanced.aspx',window,'dialogWidth:800px;dialogHeight:600px;status:0;scroll:yes');reload.click();}else{window.open('Advanced.aspx');}"><%=ResourceManager.GetString("advancedsetting")%></a></span><a id='reload' href='uploadfile.aspx' style='display:none'></a></td>
    <td align="right">
